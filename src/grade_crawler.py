...
class GradeCrawler:
    ...
    def parse_grade_data(self, html, gmgo_cd, bank_name, province='', district='', evaluation_month=12):
        ...
        try:
            ...
            if data_str:
                # 정규식으로 경영실태평가 데이터 추출
                # 패턴: 31000001 + (기관명) + | + (기준일: 2025-12-31, 2025.12.31, 2025년 12월 31일 등) + | + (등급)
                pattern = re.compile(r"31000001([^\|]+)\|([0-9가-힣\-\.\s]{8,15})\|([0-9])")
                matches = pattern.findall(data_str)
                
                if matches:
                    기관명, 기준일, 등급코드 = matches[0]
                    # 날짜에서 숫자를 제외한 모든 문자 제거 (2025년 12월 31일 -> 20251231)
                    기준일 = re.sub(r"[^0-9]", "", 기준일)
